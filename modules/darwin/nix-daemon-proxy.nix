...
let
  ...
  injectScript = ''
    ...
    # NB: never `exit` here — this text is inlined into nix-darwin's
    # single activation script (preActivation); an exit would abort the
    # whole activation on no-proxy machines.
